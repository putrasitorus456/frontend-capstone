...
const NotificationTab = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await fetch("/api/notification/combined");
        ...
        const transformedData = sortedData.map((item: any, index: any) => ({
          ...
          notification: item.title.includes("telah dinyalakan")
            ? "ON"
            : item.title.includes("telah dimatikan")
            ? "OFF"
            : "ISSUES",
          description: item.title,
          lastStatus: item.title.includes("telah dinyalakan")
            ? "ON"
            : item.title.includes("telah dimatikan")
            ? "OFF"
            : "ISSUES",
  ...
  const handleDownloadPDF = async (pageWidth = 1100, pageHeight = 800, customColumnWidths = [40, 50, 350, 50, 100, 150, 100, 100]) => {
    ...
    // Title
    page.drawText('Notification List', {
      x: 50,
      y: height - 40,
    ...
    const headers = ['INDEX', 'LAMP ID', 'LOCATION', 'TYPE', 'NOTIFICATION', 'DESCRIPTION', 'LAST STATUS', 'TRIGGERED TIME'];
    ...
    data.forEach((event) => {
      const rowData = [
        ...
        event.type,
        event.notification,
        wrapText(event.description, customColumnWidths[5]),
        event.lastStatus,
        event.triggeredTime,
      ];
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-black">Notification List</h2>
        ...
        <table className="min-w-full bg-white border border-gray-200">
          <thead>
            <tr className="bg-blue-600 text-white text-sm">
              ...
              <th className="px-2 py-1">NOTIFICATION</th>
              <th className="px-2 py-1">DESCRIPTION</th>
              <th className="px-2 py-1">LAST STATUS</th>
          ...
          <tbody>
            {paginatedData.map((event, index) => (
              <tr key={index} className={`border-t border-gray-200 text-black text-sm ${index % 2 === 0 ? 'bg-gray-300' : 'bg-white'}`}>
                <td>
                  {event.location === "Streetlights may have been removed" ? (
                    <span className="px-2 py-2 text-center"> - </span>
                  ) : (
                    <span className="px-2 py-2 text-center">{event.index}</span>
                  )}
                </td>
                <td>
                  {event.location === "Streetlights may have been removed" ? (
                    ...
                    <span className="px-2 py-2 text-center">{event.lampId}</span>
                  )}
                </td>
                <td>
                  {event.location === "Streetlights may have been removed" ? (
                ...
                <td className="px-2 py-2 flex items-center justify-center">
                  <FaCircle className={`mr-2 ${getNotificationColor(event.notification)}`} />
                </td>
                <td className="px-2 py-2 text-center">{event.description}</td>
                <td className="px-2 py-2 text-center">{event.lastStatus}</td>
